...
function FilterDropdown<T>({
  ...
  const renderPanel = () => {
    ...
    return (
      <div
        ref={panelRef}
        className="fixed z-[200] bg-[#1b1023] border border-fuchsia-700/40 rounded-[14px] shadow-2xl shadow-black/40 overflow-hidden"
      ...
      >
        <div className="max-h-[320px] overflow-y-auto overscroll-contain p-1.5">
          ...
          >
            <div className="w-3.5 h-3.5 shrink-0">{!selected && <Check className="w-3.5 h-3.5 text-fuchsia-300" />}</div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2.5 text-left rounded-[8px] transition-colors text-[13.5px] ${
                    isSelected ? "bg-fuchsia-500/15 text-white" : "text-white/85 hover:bg-white/[0.06] hover:text-white"
                  }`}
                >
                  <div className="w-3.5 h-3.5 shrink-0">
                    {isSelected && <Check className="w-3.5 h-3.5 text-fuchsia-300" />}
                  </div>
                  <span className="flex-1 truncate">{getLabel(item)}</span>
                  {typeof count === "number" && (
                    <span className={`text-[11px] tabular-nums ${isSelected ? "text-fuchsia-200" : "text-white/40"}`}>
                      {count}
                    </span>
                  )}
                </button>
  ...
  return (
    <div className="relative w-full md:flex-1">
      ...
      >
        <span className="text-fuchsia-300 shrink-0">{icon}</span>
...
export function RepetitorlarHero({ regions, groups }: Props) {
  const [selectedRegion, setSelectedRegion] = useState<Region | null>(null);
  const [selectedGroup, setSelectedGroup] = useState<Group | null>(null);

  return (
    <div className="max-w-[1600px] mx-auto px-5 md:px-20 pt-6 md:pt-8 pb-6">
      <div className="relative rounded-[16px] md:rounded-[20px] border border-[#e4e7ea] overflow-hidden bg-gradient-to-br from-[#1b1023] via-[#3a1e4f] to-[#7a3d8a]">
        {/* Wave animations */}
        <div className="absolute bottom-0 left-0 right-0 h-[60px] overflow-hidden pointer-events-none">
          <svg className="absolute bottom-0 w-[200%] h-[60px] animate-[waveMove_6s_linear_infinite]" viewBox="0 0 1440 60" preserveAspectRatio="none">
            <path d="M0,30 C180,60 360,0 540,30 C720,60 900,0 1080,30 C1260,60 1440,0 1440,30 L1440,60 L0,60 Z" fill="rgba(212,162,212,0.06)" />
          </svg>
          <svg className="absolute bottom-0 w-[200%] h-[60px] animate-[waveMove_8s_linear_infinite]" viewBox="0 0 1440 60" preserveAspectRatio="none">
            <path d="M0,35 C200,10 400,55 600,35 C800,10 1000,55 1200,35 C1400,10 1440,35 1440,35 L1440,60 L0,60 Z" fill="rgba(212,162,212,0.04)" />
          </svg>
        </div>

        {/* Subtle ambient glow */}
        <div className="absolute -top-20 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full bg-fuchsia-400/10 blur-3xl pointer-events-none" />
        ...
        <div className="relative px-5 py-8 md:px-10 md:py-12 text-center">
          ...
          <div className="inline-flex items-center gap-1.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-full px-2.5 py-1 mb-4">
            ...
            <span className="text-[11px] font-semibold text-white tracking-wider uppercase">Sizga mosini toping</span>
          </div>

          {/* H1 */}
          <h1 className="text-[29px] md:text-[39px] font-semibold text-white tracking-[-0.02em] mb-3 max-w-[820px] mx-auto leading-tight">
            O&apos;zbekistondagi repetitorlar katalogi
          </h1>

          {/* Subtitle — AEO question format */}
          <p className="text-[13px] md:text-[15px] text-white/70 max-w-[620px] mx-auto">
            Qaysi repetitor sizga mos? Tajriba, fan va reytingni solishtiring, to&apos;g&apos;ri tanlov qiling.
          </p>
          ...
          >
            ...
            <button
              type="submit"
              className="bg-fuchsia-500 hover:bg-fuchsia-400 text-white text-[14px] font-semibold rounded-[10px] px-5 py-2.5 flex items-center justify-center gap-2 transition-colors shadow-lg shadow-fuchsia-500/30 shrink-0"
            >
