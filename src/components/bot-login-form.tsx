...
interface Props {
  ...
  subtitle: string;
  botUsername: string;
  expectedRole: "provider" | "student";
  successPath: string;
  // Sibling URL shown as alternate ("If you're a teacher → /center")
  altLabel?: string;
  altHref?: string;
}

// Shared Telegram-code login UI used by /center and /student. The page passes
// the bot username, expected role and success redirect — flow is otherwise identical.
export function BotLoginForm({ title, subtitle, botUsername, expectedRole, successPath, altLabel, altHref }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res?.error) {
        setError("Kod noto'g'ri yoki bu sahifa uchun mos emas. " + (altLabel ? `Boshqa rol uchun ${altLabel} ga o'ting.` : ""));
        setSubmitting(false);
        return;
      }
    ...
    <div className="min-h-screen bg-[#f0f2f3] flex items-center justify-center px-5 py-8">
      <div className="w-full max-w-[440px]">
        ...
        <div className="bg-white rounded-[20px] border border-[#e4e7ea] p-6 md:p-8">
          <h1 className="text-[22px] md:text-[26px] font-bold text-[#16181a] tracking-[-0.02em]">{title}</h1>
          <p className="text-[13px] text-[#7c8490] mt-1.5">{subtitle}</p>
          ...
          <div className="mt-6 rounded-[14px] bg-[#eaf1fb] border border-[#7ea2d4]/30 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                >
                  <ExternalLink className="w-3.5 h-3.5" /> @{botUsername}
                </a>
              </div>
            </div>
          </div>
          ...
          {altLabel && altHref && (
            <p className="text-[11px] text-[#7c8490] text-center mt-2">
              {altLabel}: <Link href={altHref} className="text-[#7ea2d4] hover:underline">{altHref}</Link>
            </p>
          )}
        </div>
      </div>
