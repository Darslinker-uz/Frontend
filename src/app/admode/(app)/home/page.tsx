...
export default function AdminHomePage() {
  ...
  const statCards = data ? [
    { label: "Foydalanuvchilar", value: fmt(data.stats.users.total), change: `${data.stats.users.delta >= 0 ? "+" : ""}${data.stats.users.delta}%`, trend: data.stats.users.delta >= 0 ? "up" : "down", icon: Users, href: "/admode/users" },
    { label: "Aktiv e'lonlar", value: fmt(data.stats.listings.active), change: `${data.stats.listings.delta >= 0 ? "+" : ""}${data.stats.listings.delta}%`, trend: data.stats.listings.delta >= 0 ? "up" : "down", icon: FileText, href: "/admode/listings" },
    { label: "Oylik daromad", value: fmtMoney(data.stats.revenue.thisMonth), change: `${data.stats.revenue.delta >= 0 ? "+" : ""}${data.stats.revenue.delta}%`, trend: data.stats.revenue.delta >= 0 ? "up" : "down", icon: CreditCard, href: "/admode/payments" },
    { label: "Konversiya", value: `${data.stats.conversion.rate}%`, change: `${data.stats.conversion.delta >= 0 ? "+" : ""}${data.stats.conversion.delta}%`, trend: data.stats.conversion.delta >= 0 ? "up" : "down", icon: TrendingUp, href: "/admode/analytics" },
  ] : [];

  const urgentTasks = data ? [
    { label: "Moderatsiya kutayotgan e'lonlar", count: data.urgent.pendingListings, href: "/admode/listings?status=pending", icon: FileText },
    { label: "Yangi o'quvchi leadlari", count: data.urgent.newStudentLeads, href: "/admode/leads?tab=students", icon: Users },
    { label: "Hamkorlik so'rovlari", count: data.urgent.newPartnerApps, href: "/admode/leads?tab=hamkorlik", icon: Building2 },
    { label: "Yordam so'rovlari", count: data.urgent.newHelpReqs, href: "/admode/leads?tab=yordam", icon: MessageSquare },
  ] : [];
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-[16px] p-5 md:p-6" style={{ backgroundColor: config.surface, border: `1px solid ${config.surfaceBorder}` }}>
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/admode/analytics" className="text-[13px] font-medium flex items-center gap-1" style={{ color: config.accent }}>
