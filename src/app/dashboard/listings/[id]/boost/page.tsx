...
export default function BoostPage() {
  ...
  return (
    <div className="px-3 sm:px-5 md:px-8 py-6 md:py-8 pb-24 md:pb-8">
      <Link href="/dashboard/boost" className="inline-flex items-center gap-2 text-[13px] text-[#7ea2d4] font-medium mb-6">
      ...
      {selectedClass && (
        <>
          ...
          <div className="rounded-[14px] p-4 mb-5 flex items-center justify-between" style={{ backgroundColor: config.surface, border: `1px solid ${config.surfaceBorder}` }}>
            ...
            {!canAfford && (
              <Link href="/dashboard/balance" className="h-[36px] px-4 rounded-[10px] text-[13px] font-medium leading-[36px]" style={{ backgroundColor: config.accent, color: config.accentText }}>
                To&apos;ldirish
              </Link>
            )}
          </div>
